...
class NetworkSimulator:
    ...
    def runSimulator(self):
        ...
        while True:
            ...
            if self.TRACE > 1:
                print("\nMAIN: receive event, time=" +
                      str(next_event.event_time) + "at "+str(next_event.entity))
                ...
                if (next_event.event_type == EventType.FROM_LAYER2):
                    ...
                    print("Node " + str(next_event.rtpacket.destid) +
                          " Received a packet from Node " + str(next_event.rtpacket.sourceid))
            ...
            if next_event.event_type == EventType.FROM_LAYER2:
                if next_event.entity >= 0 and next_event.entity < self.NUM_NODES:
                    ...
                    changed = self.nodes[next_event.entity].recvUpdate(
                        next_event.rtpacket)
                    # if changed:
        ...
        print("\nSimulator terminated at t=" +
              str(self.clocktime)+" no packets in medium\n")
    ...
    def tolayer2(self, packet):
        ...
        if self.TRACE > 1:
            print("\nTOLAYER2: source: " + str(mypacket.sourceid) +
                  " dest: " + str(mypacket.destid) + ": costs: ", end="")
            for i in range(self.NUM_NODES):
